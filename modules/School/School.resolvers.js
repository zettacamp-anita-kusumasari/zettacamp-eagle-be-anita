...
async function getAllSchools() {
  try {
    ...
    throw new ApolloError(`Failed to fetch schools: ${error.message}`, "INTERNAL SERVER ERROR");
  }
}
...
async function getOneSchool(_, { id }, context) {
  ...
  try {
    ...
    if (!school) {
      throw new ApolloError("School not found", "NOT FOUND");
    }
    // *************** Return the fetched and validated school object
    return school;
  } catch (error) {
    // *************** If any unexpected error occurs, throw a generic internal server error
    throw new ApolloError(`Failed to fetch school: ${error.message}`, "INTERNAL SERVER ERROR");
  }
}
 ...
 *
 * This resolver function validates the input using `validateSchool`, attaches a default `createdBy` user ID
...
async function createSchool(_, { input }) {
  try {
    ...
    validatedInput.createdBy = '6847af632c3aafcd7ad64244';
    ...
    throw new ApolloError(`Failed to create school: ${error.message}`, 'SCHOOL CREATION FAILED');
  }
}
...
async function updateSchool(_, { id, input }) {
  ...
  try {
    ...
    throw new ApolloError(`Failed to update school: ${error.message}`, 'SCHOOL UPDATE FAILED');
  }
}

/**
 * Soft-deletes a school by its ID by setting `deletedBy` and `deletedAt` fields.
 *
 * This function performs the following steps:
 * - Validates the provided `id` to ensure it's a valid MongoDB ObjectId.
 * - Attempts to update the corresponding school document with `deletedBy` and `deletedAt` fields.
...
async function deleteSchool(_, { id }) {
  ...
  try {
    ...
    const deletedBy = '6846e5769e5502fce150eb67';
    ...
    throw new ApolloError(`Failed to delete school: ${error.message}`, 'SCHOOL DELETION FAILED');
  }
}
...
async function studentsLoader(parent, _, context) {
  try {
    ...
    throw new ApolloError(`Failed to load students: ${error.message}`, "STUDENT LOAD FAILED");
  }
}
 ...
 *
 * Uses DataLoader to efficiently retrieve the user document associated with the `createdBy` field
 * from the parent object. This helps avoid redundant database queries in GraphQL.
 *
 * @async
 * @function createdByLoader
 * @param {Object} parent - The parent object that contains the `createdBy` field (a user ID).
...
async function createdByLoader(parent, _, context) {
  try {
    // *************** Use DataLoader to fetch the user based on the `createdBy` field from the parent object
 ...
 *
 * - Retrieves the `deletedBy` field from the parent object.
 * - Uses `context.userLoader` (a DataLoader instance) to fetch the corresponding user.
 * - Returns `null` if the entity was not deleted (i.e., `deletedBy` is undefined or null).
 * - Throws an ApolloError if the user loading process fails.
 *
 * @async
 * @function deletedByLoader
 * @param {Object} parent - The parent object containing the `deletedBy` user ID (optional).
...
async function deletedByLoader(parent, _, context) {
  try {
    // *************** If the entity has a deletedBy value, load the associated user; otherwise, return null
